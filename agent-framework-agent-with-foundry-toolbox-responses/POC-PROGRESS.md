...
Last updated: 2026-08-31 (aks_check_storage optimized: 5 Run Command calls → 1; unit tests written, pytest execution and real-cluster re-validation still pending)
...
- **Current phase:** Phase 9 (Pre-Upgrade Assessment) is the most advanced; `aks_check_deprecated_apis` is now the first MCP tool with full end-to-end validation (unit + real cluster). Phase 2/5 tool completeness overall remains the blocker (9 of 10 tools still lack unit tests, only 1 of 10 has real-cluster validation).
- **Current task:** With `aks_check_deprecated_apis` fully validated, the next focus is either extending unit-test coverage to `tools/discovery.py`/`tools/validation.py`, or Phase 8 (real MCP endpoint) wiring.
- **Overall POC status:** All 10 planned MCP tools are implemented. `aks_check_deprecated_apis` is now **fully validated and marked 🟢 DONE**: unit tests `28 passed in 1.78s`; real-cluster run against `AKS_oldapp_cluster` (Kubernetes `1.34.10`) confirmed `run_command_invocations: 1`, elapsed `33.916661s` vs. the `546.12s`/17-call baseline (~93.8% faster), and the correct final result — `deprecated_api_health: INCOMPLETE` (not `HEALTHY`), `findings: []`, `blockers: []`, `warnings: []`, `query_errors: 17`, with the accurate recommendation "Deprecated API assessment is incomplete because some API queries could not be executed." `INCOMPLETE` is the correct, intentional result here: on this Kubernetes `1.34.10` cluster, the old (pre-removal) API endpoints are no longer servable at all, so the tool cannot confirm through those endpoints whether any legacy objects still exist in storage — an unavailable old API is neither evidence of usage nor proof of its absence. The POC as a whole, and real-cluster validation for the other 9 tools, remain incomplete.
- **What is working:** 10 MCP tools implemented; readiness folds in storage + deprecated-API health; `aks_check_deprecated_apis` has full unit + real-cluster validation and is the fastest, most trustworthy tool in the set.
- **What is incomplete:** live endpoint wiring (Phase 8), real cluster test scenarios (Phase 10) for the other risk areas, unit tests for `tools/discovery.py`/`tools/validation.py`, real-cluster validation for `aks_get_cluster_details`/`aks_get_node_pools`/`aks_get_available_upgrades`/`aks_check_node_health`/`aks_check_pod_health`/`aks_check_pdb`/`aks_upgrade_node_pool`/`aks_check_storage`/`aks_validate_upgrade_readiness`, and `aks_check_pod_health`'s output-size limit problem.
- **What we are doing next:** Add unit tests for `tools/discovery.py` and `tools/validation.py` (currently the biggest test-coverage gap), then proceed toward Phase 8 (real MCP endpoint) wiring.
...
> Note: on 2026-08-31 the full MCP unit test suite (28 tests: storage 8 + readiness 6 + deprecated-APIs 14) was executed and reported `28 passed in 1.78s`. Combined with the real-cluster run above (performance AND correctness both confirmed), `aks_check_deprecated_apis` is the first tool in this project with full end-to-end validation. Interpretation guardrail (do not violate): an unavailable/unqueryable old API endpoint must never be read as evidence that deprecated API usage exists, and must never be read as proof that no such usage exists — it only means usage could not be confirmed through that endpoint, which is exactly what `query_errors` + `INCOMPLETE` communicate.
...
1. Add unit tests for `tools/discovery.py` and `tools/validation.py` (currently the largest remaining test-coverage gap — 9 of 10 tools still have no unit tests).
2. Address `aks_check_pod_health`'s output-size problem (cluster-wide query exceeded the 524,288-byte Run Command limit) — the only other confirmed real-cluster defect found so far.
3. Replace the `<AKS_MCP_ENDPOINT>` placeholder in `toolbox.yaml` with a real, deployed Container App URL once one is confirmed to exist and be reachable.
4. Decide on and execute removal/retirement of the legacy Azure Functions hosting path (`function_app.py` + related Bicep resources), or explicitly document why it's being kept.
5. Verify the exact ARM action required for `agent_pools` write operations and add the corresponding RBAC role before any real (non-dry-run) upgrade is attempted.
6. Confirm the Kubernetes RBAC bootstrap scripts have actually been run against the real cluster, and capture that evidence here.
