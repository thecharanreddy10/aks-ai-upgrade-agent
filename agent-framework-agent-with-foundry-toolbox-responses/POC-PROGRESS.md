...
Last updated: 2026-08-31 (aks_check_deprecated_apis: correctness bug FIXED in code + tests; pytest execution and real-cluster re-validation of the fix are still pending)
...
- **Completed:** 10 tools now implemented and registered identically in both `main.py` and `function_app.py`: `aks_get_cluster_details`, `aks_get_node_pools`, `aks_get_available_upgrades`, `aks_check_node_health`, `aks_check_pod_health`, `aks_check_pdb`, `aks_validate_upgrade_readiness`, `aks_upgrade_node_pool`, `aks_check_storage`, `aks_check_deprecated_apis` (added 2026-08-31). The full unit test suite (22 tests) was executed on the VM on 2026-08-31: `22 passed in 4.12s`. Real-cluster performance diagnosis (2026-08-31) surfaced a severe bottleneck in `aks_check_deprecated_apis` (17 sequential AKS Run Command calls, 546.12s) and `aks_check_pod_health` (cluster-wide output exceeding the 524,288-byte Run Command limit); `aks_check_deprecated_apis` was redesigned to use a single batched Run Command and **the fix was validated against the real cluster (`AKS_oldapp_cluster`): 1 Run Command call, 34.3451432s (~34.35s), a ~94% latency reduction from the 546.12s baseline.**
- **Remaining:** The real-cluster run also surfaced a **correctness bug**: several old API versions returned "API not available/servable ... usage could not be confirmed" (kubectl exit code 1) while the tool's own `recommendations` simultaneously stated "No deprecated or removed Kubernetes API usage detected" — a contradictory, misleading result. **This bug has been fixed in code** (2026-08-31): `determine_deprecated_api_health` now returns a new `"INCOMPLETE"` state when there are no confirmed findings but some entries could not be checked, and the "no usage detected" recommendation is never emitted while `query_errors` is non-empty. **The fix itself has NOT yet been validated** — pytest has not been executed this session, and the fix has not been re-run against the real cluster. `tools/discovery.py` and `tools/validation.py` still have no dedicated unit tests. The `aks_check_pod_health` output-size problem remains unaddressed.
...
- **Completed:** Unit tests exist for storage classification (8 tests), readiness/storage integration (6 tests), and deprecated-API detection (originally 8 tests; rewritten 2026-08-31 for the batched implementation, see below) — 22 passed on the VM as of 2026-08-31 (pre-optimization code). Real-cluster performance measurements were captured for all 5 non-trivial tools; the dominant bottleneck (`aks_check_deprecated_apis`, 17 Run Command calls / 546.12s) was redesigned and **the redesign was validated against the real cluster: 1 call, 34.35s, ~94% faster.**
- **Remaining:** The rewritten `aks_check_deprecated_apis` unit tests (now 14 tests, updated 2026-08-31 for the correctness fix) still have **not** been executed via pytest in this session. The correctness bug found during the real-cluster run **has been fixed in code**, but the fix has **not** been re-validated (neither by pytest nor by a real-cluster re-run) — the tool must NOT be marked fully complete until both happen. `aks_check_pod_health`'s output-size problem (524,288-byte limit exceeded) remains unaddressed. No tests exist for discovery/validation (node/pod/pdb) or the upgrade write path.
...
- **Current phase:** Phase 9 (Pre-Upgrade Assessment) is the most advanced; Phase 2/5 (tool completeness + correctness validation) is the current blocker for moving further.
- **Current task:** Execute the full pytest suite and re-run real-cluster validation of `aks_check_deprecated_apis` to confirm the correctness fix actually resolves the misclassification — neither has happened yet this session.
- **Overall POC status:** All 10 planned MCP tools are implemented. `aks_check_deprecated_apis`'s Run Command batching optimization remains **performance-validated against the real cluster** (`AKS_oldapp_cluster`): 1 call vs. 17, 34.3451432s vs. 546.12s, a ~94% latency reduction. The correctness bug found on that real run (misleading "no findings" result while `query_errors` was non-empty) **has been fixed in code**: `determine_deprecated_api_health` now returns `"INCOMPLETE"` instead of `"HEALTHY"` when findings are empty but query_errors are not, and the "no usage detected" recommendation is gated on `query_errors` being empty too. **This fix has NOT been validated** — pytest was not executed this session (attempt unavailable/skipped) and no real-cluster re-run has occurred. **`aks_check_deprecated_apis` is therefore still NOT fully DONE.** The POC as a whole and real-cluster/end-to-end validation remain incomplete.
- **What is working:** 10 MCP tools implemented; readiness folds in storage + deprecated-API health; prior unit suite (22 tests, pre-optimization) passed on the VM; the Run Command batching optimization is confirmed working and fast on the real cluster; the correctness bug has a code fix in place.
- **What is incomplete:** live endpoint wiring (Phase 8), real cluster test scenarios (Phase 10), unit tests for `tools/discovery.py`/`tools/validation.py`, the updated `test_deprecated_apis.py` (14 tests) has not been executed via pytest this session, **the correctness fix has not been re-validated against the real cluster**, and `aks_check_pod_health`'s output-size limit problem is still unresolved.
- **What we are doing next:** Get the full pytest suite executed (VM) and get the fixed `aks_check_deprecated_apis` re-run against the real cluster (`AKS_oldapp_cluster`) to confirm both the fix and the 1-call/~34.35s performance still hold.
...
- `aks_check_deprecated_apis`'s Run Command batching optimization remains **performance-validated on the real cluster** (`AKS_oldapp_cluster`): 1 call vs. 17, 34.3451432s vs. 546.12s (~94% faster).
- **Correctness fix implemented (2026-08-31):** `determine_deprecated_api_health` now returns `"INCOMPLETE"` (not `"HEALTHY"`) when checks are inconclusive, and the tool never claims "no usage detected" while `query_errors` is non-empty. A confirmed BLOCKER/WARNING finding still takes precedence and is never suppressed by an unrelated unavailable API elsewhere in the matrix.
- **Not yet done:** pytest has not been executed this session, and the fix has not been re-run against the real cluster. Do not treat `aks_check_deprecated_apis` as fully complete until both happen.
- Next: execute the full pytest suite (28 tests total across `test_storage.py`, `test_upgrade_readiness.py`, `test_deprecated_apis.py`), then re-run `aks_check_deprecated_apis` against `AKS_oldapp_cluster` to confirm the fix and reconfirm the 1-call/~34.35s performance.
...
| `aks_check_deprecated_apis` | Detect deprecated/removed Kubernetes API usage vs. a target version | 🟡 IN PROGRESS — Run Command batching redesign (2026-08-31) is **performance-validated on the real cluster**: 1 call vs. the previous 17, 34.3451432s vs. 546.12s (~94% faster). **A correctness bug found in that same real run has been FIXED IN CODE** (2026-08-31): `determine_deprecated_api_health` now returns `"INCOMPLETE"` (not `"HEALTHY"`) when findings are empty but `query_errors` is not, and the misleading "no usage detected" recommendation is gated on `query_errors` being empty too | 🟡 Performance: 🟢 confirmed on real cluster (`AKS_oldapp_cluster`, 2026-08-31). Correctness: 🟡 FIXED IN CODE but NOT YET RE-VALIDATED — unit tests updated (14 tests) but not executed via pytest this session; no real-cluster re-run of the fix has occurred |
...
| Deprecated/removed Kubernetes APIs | 🟢 | 🟡 | `aks_check_deprecated_apis`: batching optimization performance-validated on the real cluster (1 call, 34.35s, ~94% faster than the 17-call/546.12s baseline). **Correctness bug found on that same real run has been FIXED IN CODE** (`INCOMPLETE` health state + gated recommendation) but the fix is **NOT yet re-validated** by pytest or a real-cluster re-run |
...
| No deprecated APIs in use for target version | `HEALTHY`, no findings, 1 Run Command call | Unit test asserts this (`test_deprecated_apis.py::test_no_deprecated_apis_detected`); also asserts the "no usage detected" recommendation IS present when there are truly no findings/errors | 🟡 updated, not yet executed |
...
| API unavailable, no findings elsewhere → INCOMPLETE, not HEALTHY | Unavailable API surfaces as a query error; overall health is `INCOMPLETE`; "no usage detected" recommendation is NOT emitted | Unit test asserts this (`test_api_unavailable_is_distinguished_from_no_objects_found`, updated for the fix) | 🟡 updated, not yet executed |
| API unavailable alongside a real BLOCKER/WARNING finding elsewhere | The real finding is still reported (BLOCKED/WARNING); `query_errors` and an "incomplete" recommendation are ALSO present, not hidden | Unit test asserts this (`test_query_errors_alongside_real_findings_preserve_both`, new) | 🟡 new, not yet executed |
...
| `determine_deprecated_api_health` 3-arg classification (incl. INCOMPLETE) | HEALTHY/WARNING/BLOCKED/INCOMPLETE all correctly derived from (blockers, warnings, query_errors) | Unit test asserts this (`test_determine_deprecated_api_health`, extended) | 🟡 updated, not yet executed |
...
| Deprecated API usage (real cluster) — performance | 1 Run Command call, fast execution | **Actual: 1 call, 34.3451432s (~34.35s), vs. 17 calls/546.12s baseline (~94% faster). Executed against `AKS_oldapp_cluster`.** | 🟢 CONFIRMED — performance goal met |
| Deprecated API usage (real cluster) — correctness | Findings/recommendations accurately reflect what could and could not be confirmed | Bug found 2026-08-31 (see [Section 12](#12-problems--blockers)); **fix implemented in code the same day, but NOT yet re-run against the real cluster** | 🟡 FIX IMPLEMENTED, RE-VALIDATION PENDING — do not mark complete until a real-cluster re-run confirms the fixed output |
...
> Note: on 2026-08-31 the user ran the full MCP unit test suite (pre-optimization code) on the VM and reported `22 passed in 4.12s`. The `aks_check_deprecated_apis` tests were then rewritten for the batched implementation and updated again the same day for the correctness fix (14 tests now in that file; 28 total across the suite) and have **not** been re-executed. Do not read the rows above marked "not yet executed" as passing until a real pytest run confirms it. Separately, the real-cluster run confirmed the performance fix works and confirmed a correctness bug; that bug has a code fix in place but has not been re-run against the real cluster to confirm the fix actually resolves it.
...
1. **Execute the full pytest suite** (VM), including the updated `test_deprecated_apis.py` (14 tests; 28 total across the suite), and record the actual result — the correctness fix has not been confirmed by test execution yet.
2. **Re-run `aks_check_deprecated_apis` ONLY against the real cluster** (`AKS_oldapp_cluster`, subscription `bb0e2c9e-d7fb-45e4-92cf-654f380e6388`, resource group `AKS_Upgrade_agent`) to confirm the correctness fix actually resolves the misclassification, and reconfirm the 1-call/~34.35s performance still holds. **Only after both #1 and #2 pass should `aks_check_deprecated_apis` be marked 🟢 DONE.**
...
11. Once the correctness fix is confirmed (test + real-cluster), re-measure full readiness (`check_mode="full"`) end-to-end — previously estimated at ~13 minutes, now expected to drop substantially given the confirmed 17→1 Run Command reduction.
