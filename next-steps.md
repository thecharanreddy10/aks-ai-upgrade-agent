...
### CI smoke-test pipeline for AKS MCP

A GitHub Actions workflow is included at `.github/workflows/aks-mcp-smoke-test.yml`.

What it does:

1. Runs on-demand (`workflow_dispatch`) or every 12 hours.
2. Executes `scripts/validate-aks-mcp-tools.ps1` against your deployed MCP endpoint.
3. Fails the run if any of the six MCP tools fail.

Repository variables to set:

1. `AKS_MCP_ENDPOINT` (for example `https://aks-mcp.<env>.azurecontainerapps.io/mcp`)
2. `AKS_SUBSCRIPTION_ID`
3. `AKS_RESOURCE_GROUP`
4. `AKS_CLUSTER_NAME`
5. `AKS_NAMESPACE` (optional, defaults to `default`)

Run it manually from Actions:

1. Open workflow **AKS MCP Smoke Test**.
2. Choose **Run workflow**.
3. Optionally override endpoint/subscription/resource-group/cluster/namespace via inputs.
